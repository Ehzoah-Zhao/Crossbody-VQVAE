...
# ================= 核心还原逻辑 V4 (无物理硬锁版) =================
def recover_g1_motion_v4(features, mean=None, std=None):
    """
    从 V4 特征 (363维) 还原出全局 3D 坐标和全局四元数旋转。
    专为下游强化学习追踪控制器提供平滑的 Reference Motion。
    """
    ...
    # 自动推断总节点数 J (适应 363 维)
    assert (D - 3) % 12 == 0, f"特征维度 {D} 异常，无法匹配 V4 提取规则！"
    J = (D - 3) // 12
    ...
    rot_data = features[..., idx4:idx5].view(B, T, J, 6) # J 个关节的相对旋转
    ...
    # 2. 还原所有关节的全局旋转 (融合骨盆真实的 Pitch 和 Roll)
...
if __name__ == "__main__":
    B, T, D = 2, 10, 363 # G1 30个关节 -> 363维
    dummy_features = torch.randn(B, T, D)
    
    positions, rotations = recover_g1_motion_v4(dummy_features)
    print(f"✅ 成功还原 V4 特征 (纯净版)！")
    print(f"全局坐标 Shape: {positions.shape}  (B, T, J, 3)")
    print(f"全局旋转 Shape: {rotations.shape}  (B, T, J, 4)")