"""
train_vq_v4_quat.py - Cross-Embodiment VQ-VAE (v5: quaternion rotations, no ortho reg)

P0: 363-dim G1 + FK physical losses + CUT unpaired contrastive
P1: Physics metrics monitoring
P2: REMOVED - Quaternion rotations are naturally orthogonal

Usage:
  python train_vq_v4_quat.py --data_dir_A ./dataset/smpl/new_joint_vecs 
...
from utils.physics_losses import FKPositionLossQuat, PhysicsMetrics, G1_303_SLICES, G1_N_JOINTS
...
def get_weights(nb_iter, warm_up_iter):
    ...
    Stage 3 (10k+):       + FK physical losses + paired contrastive
    """
    if nb_iter < warm_up_iter:
        ...
        # Quaternion rotations are naturally orthogonal - no ortho_reg needed!
...
if __name__ == "__main__":
    ...
    fk_loss_fn = FKPositionLossQuat(supervised_weight=0.8, self_consist_weight=0.3, ee_weight=1.5).cuda()
    ...
    # ========== 6. Training Loop ==========
    logger.info("Starting training (303-dim quat + FK + CUT)...")
    nb_iter = start_iter
    ...
    while nb_iter < args.total_iter:
        ...
        # Orthogonal reg removed: quaternions are naturally SO(3)
